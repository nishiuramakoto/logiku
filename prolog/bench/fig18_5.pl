...
plan( StartState, Goals, Plan)  :-
   ...
   conc( _, [Plan], SolutionPath).  			% Working plan is last element of solution path
...
s( pop( Acts, [Cond:Time | OpenPs], TrueConds, Fin), pop( Acts, OpenPs, TrueConds, Fin))  :-  
...
s( pop( Acts, [Cond:Time | OpenPs0], TrueConds0, Fin), 
   ...
   add_conds( [ Cond:Time1/Time2 | RestEffectsTimes], TrueConds0, TrueConds),  
          % Add effects to TrueConds0
   conc( NewOpenPs, OpenPs0, OpenPs).		% Add preconditions of Action to goals
...
no_conflict1( CondA:Ta1/Ta2, CondB:Tb1/Tb2)  :-
  ...
  ( Ta2 #=< Tb1; Tb2 #=< Ta1 )	% Ensure no time overlap between CondA and ConB
  ;
...
depth_first( First, [First | Rest], MaxActions)  :-	
  ...
  ( NActs < MaxActions, !	       % # actions in plan is below MaxActions	
    ; 
    Second = pop( Acts, _, _, _) ),     % # actions in plan at maximum, no new action may be added to plan	
  s( First, Second),
...
show_pop( pop( Actions, _, _, _))  :-
     ...
     nl, write('Actions = '), write( Sorted), fail	% Write schedule and backtrack to produce other schedules
     ; 
