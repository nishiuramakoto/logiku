...
週法定労働時間(Case, H) :-  要件(Case, 特例対象事業(Case)) , !,
			    H is 44
			    ;
			    H is 40.

日法定労働時間(_Case, H) :-  H is 8.



労働時間(Case) :- 要件(Case,事業場外労働(Case)) , !,
                  事業場外労働のみなし労働時間(Case).
...
労働時間(Case,WP, H) :- 事業場外労働のみなし労働時間(Case,WP,H) ,!.
労働時間(Case,WP, H) :- 自然数要件(Case, 問(この事業場における労働時間(WP)) , H).

事業場(Case,WP) :- リスト要件(Case, 労働する事業場をリストしてください, WP).
